        ...
        foreach (Ship ship in shipsToMove) {
            ...
            foreach (Command command in shipAI.commands) {
                if (command.commandType != Command.CommandType.Move || !(Vector2.Distance(command.targetPosition,
                        objectsToMoveTo[objectIndex].GetPosition()) <=
                    ship.GetSize() + objectsToMoveTo[objectIndex].GetSize())) continue;
