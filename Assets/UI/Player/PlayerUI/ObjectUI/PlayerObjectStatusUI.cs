...
public class PlayerObjectStatusUI : MonoBehaviour {
    ...
    private void RefreshUIForUnit(UnitUI unitUI) {
        ...
        objectImage.color = new Color(unitUI.unitIconUI.GetColor().r, unitUI.unitIconUI.GetColor().g,
            unitUI.unitIconUI.GetColor().b, 1);
    ...
    public void RefreshPlayerObjectStatusUI(FleetUI fleetUI, UnitUI unitUI, int unitCount) {
        ...
        healthText.GetComponent<TMP_Text>().text = "Hull " +
            NumFormatter.ConvertNumber(fleetUI.fleet.GetFleetHealth()) + "/" +
            NumFormatter.ConvertNumber(fleetUI.fleet.GetMaxFleetHealth());
        shieldText.GetComponent<TMP_Text>().text = "Shields " +
            NumFormatter.ConvertNumber(fleetUI.fleet.GetFleetShields()) + "/" +
