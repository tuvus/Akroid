...
public class PlayerObjectStatusUI : MonoBehaviour {
	...
	private void RefreshUIForUnit(Unit unit) {
        ...
        healthText.GetComponent<TMP_Text>().text = "Hull " + NumFormatter.ConvertNumber(unit.GetHealth()) + "/" + NumFormatter.ConvertNumber(unit.GetMaxHealth());
        shieldText.GetComponent<TMP_Text>().text = "Shields " + NumFormatter.ConvertNumber(unit.GetShields()) + "/" + NumFormatter.ConvertNumber(unit.GetMaxShields());
    }
    ...
    public void RefreshPlayerObjectStatusUI(Fleet fleet, Unit unit, int unitCount) {
        ...
        float sizeRatio = objectImage.rectTransform.sizeDelta.y / objectImage.rectTransform.sizeDelta.x;
        objectImage.rectTransform.sizeDelta = new Vector2(70 / sizeRatio, 70);
        healthText.GetComponent<TMP_Text>().text = "Hull " + NumFormatter.ConvertNumber(fleet.GetFleetHealth()) + "/" + NumFormatter.ConvertNumber(fleet.GetMaxFleetHealth());
        shieldText.GetComponent<TMP_Text>().text = "Shields " + NumFormatter.ConvertNumber(fleet.GetFleetShields()) + "/" + NumFormatter.ConvertNumber(fleet.GetMaxFleetShields());
