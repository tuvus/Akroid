...
public class BuildShipsAtStationUICondition : UIWrapperEventCondition<BuildShipsAtStation> {
    ...
    public override void GetVisualizedObjects(List<ObjectUI> objectsToVisualise, List<Button> buttonsToVisualize) {
        var shipBlueprintsToBuild = new List<Ship.ShipBlueprint>(conditionLogic.shipBlueprintsToBuild);
        conditionLogic.station.moduleSystem.Get<ConstructionBay>().ForEach(c =>
            c.buildQueue.ForEach(constructionBayBlueprint => {
