...
public class FleetFormation {
    ...
    public static (List<Ship>, List<Vector2>) GetFormationShipPosition(Fleet fleet, Vector2 position, float rotation,
        float spread,
    ...
    private static (List<Ship>, List<Vector2>) GetLineFormation(List<Ship> shipsSorted, Fleet fleet, Vector2 position,
        float rotation,
        float spread) {
        var shipPositions = new List<Vector2>();
        float shipSize = fleet.GetMaxShipSize();
        Vector2 startPosition =
            position - Calculator.GetPositionOutOfAngleAndDistance(rotation - 90,
                spread + shipSize * fleet.GetShips().Count);
        Vector2 endPosition =
            position - Calculator.GetPositionOutOfAngleAndDistance(rotation + 90,
                spread + shipSize * fleet.GetShips().Count);
    ...
    private static (List<Ship>, List<Vector2>) GetOvalFormation(List<Ship> shipsSorted, Fleet fleet, Vector2 position,
        float rotation,
        ...
        for (int i = 0; i < shipsSorted.Count; i++) {
            Vector2? newPosition =
                FindFreeShipLocationIncrement(positionGiver, shipsSorted, shipPositions, scalar, ref random);
    ...
    public static Vector2? FindFreeShipLocationIncrement(PositionGiver positionGiver, List<Ship> shipsSorted,
        List<Vector2> shipPositions,
        ...
        while (true) {
            Vector2? targetPosition = FindFreeShipLocation(positionGiver, distance,
                distance + positionGiver.incrementDistance, shipsSorted,
    ...
    public static Vector2? FindFreeShipLocation(PositionGiver positionGiver, float minRange, float maxRange,
        List<Ship> shipsSorted,
        ...
        for (int i = 0; i < positionGiver.numberOfTries; i++) {
            ...
            Vector2 tryPos = positionGiver.position +
                Calculator.GetPositionOutOfAngleAndDistance(angle, distance) * scalar;
    ...
    public static bool ConfirmShipLocation(Vector2 pos, float distanceFromObject, List<Ship> shipsSorted,
        List<Vector2> shipPositions) {
